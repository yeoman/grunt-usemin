...
//
var get_blocks = function(content) {
  ...
  lines.forEach(function(l) {
    ...
    if(block && last) {
      ...
      if (asset && asset[2]) {
        ...
        if(main) {
          last.rjs = last.rjs || {};
          last.rjs.dest = last.dest;
          last.rjs.name = main[1];
...
var HTMLProcessor = module.exports = function(filepath, content, revvedfinder) {
  this.filepath = filepath;
  this.content = content;
  this.revvedfinder = revvedfinder;
  this.linefeed = /\r\n/g.test(content) ? '\r\n' : '\n';
  this.blocks = get_blocks( this.content );
};

//
// Return the string that will replace the furnished block
//
HTMLProcessor.prototype.replace_with = function replace_with( block ) {
  var result;

  if (block.type === 'css') {
    result = block.indent + '<link rel="stylesheet" href="' + block.dest + '"\/>';
  } else if (block.type === 'js') {
    result = block.indent + '<script src="' + block.dest + '"><\/script>';
  } else {
    result = '';
  }
  return result;
...
//
HTMLProcessor.prototype.replace_blocks = function replace_blocks() {
  var result = this.content;
  var self = this;

  this.blocks.forEach(function(b){
    var block_line = b.raw.join( self.linefeed );
    result = result.replace(block_line, self.replace_with( b ));
...
//
HTMLProcessor.prototype.replace_with_revved = function replace_with_revved(lines) {
    ...
    regexps.forEach(function(rx) {
      content = content.replace( rx, function(match, src) {
...
HTMLProcessor.prototype.process = function process() {
  ...
  return this.replace_with_revved ( this.replace_blocks() );
};
