...
// +expand_fn+ : a function that will return a list of file matching a given pattern (for example grunt.file.expand)
//
var RevvedFinder = module.exports = function(expand_fn) {
    this.expand_fn = expand_fn;
};
...
RevvedFinder.prototype.find = function find(ofile, basedir) {
    ...
    var filepaths = this.expand_fn(path.join('**/*') + basename);
    ...
    return start_at_root ? '/' + filename : filename;
};
